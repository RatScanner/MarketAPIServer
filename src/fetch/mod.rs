...
pub async fn fetch() -> Result<Vec<models::Item>, FetchError> {
    ...
    let body = json!({
        "query": "{items(type: any) {id,basePrice,updated,iconLink,wikiLink,imageLink,avg24hPrice,lastLowPrice,sellFor {price,vendor {name, ...on TraderOffer {trader {id, name}}}}}}",
    });
